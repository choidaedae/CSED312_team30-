...
void swap_in(size_t used_index, void *kaddr)
{
    ...
    int i = 0;
    for (i = 0; i < 8; i++) block_read(block_swap, used_index * 8 + i, kaddr + BLOCK_SECTOR_SIZE * i);
...
size_t swap_out(void *kaddr)
{
    ...
    int i;
    for (i = 0; i < 8; i++) block_write(block_swap, index_swap * 8 + i, kaddr + BLOCK_SECTOR_SIZE * i);
